...
def validaConta(userName, userPass, Logwindow):
    """
    Validar autenticação com uma conta
    """
...
def Admin_info(userName,estado):
    """
    ...
    admin_linha = userName + ";" + str(today) + ";" + str(current_time) + ";" + estado + "\n"       #adiciona linha com os parametros separados por ; e adiciona \n no final para manter formatação do ficheiro
...
def Adicionar_Categorias(Nome_categoria): 
    """
        Adicionar categoria ao ficheiro e à lista interna
    """
    if Nome_categoria == "":
...
def Remover_Categorias(Nome_Categoria):
    """
        Remove categoria do ficheiro e da lista interna
    """
    if Nome_Categoria == "":
        messagebox.showerror("Categorias", "Não colocou nada para adicionar!")
    else:
